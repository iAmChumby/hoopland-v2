...
class TestGetPlayer:
    ...
    def test_get_player_exists(self, db_session):
        ...
        # Add a player
        player = Player(
            source_id="12345", league="NBA", season="2023-24", name="Test Player"
        )
    ...
    def test_get_player_wrong_season(self, db_session):
        ...
        player = Player(
            source_id="12345", league="NBA", season="2022-23", name="Test Player"
        )
        db_session.add(player)
        db_session.commit()
...
class TestSyncNBASeasonStats:
    ...
    def test_sync_nba_season_stats_skips_if_cached(self, mock_client_class, db_session):
        ...
        for i in range(450):
            player = Player(
                source_id=str(i), league="NBA", season="2023-24", name=f"Player {i}"
            )
            db_session.add(player)
        db_session.commit()
...
class TestSyncNCAASeasonStats:
    ...
    def test_sync_ncaa_tournament_mode(self, mock_espn_class, db_session):
        """Test NCAA sync in tournament mode limits to 64 teams with game_id."""
        mock_client = MagicMock()
        mock_espn_class.return_value = mock_client

        # Create 64 mock teams with game_id (required for tournament mode)
        mock_teams = [
            {
                "id": str(i),
        ...
        mock_client.get_tournament_teams.return_value = mock_teams
        mock_client.get_game_roster.return_value = []  # Empty roster for test
    ...
    def test_sync_ncaa_full_mode(self, mock_espn_class, db_session):
        ...
        mock_teams = [
            {"id": str(i), "displayName": f"Team {i}", "slug": f"team-{i}"}
            for i in range(100)
        ]
...
class TestBackfillAppearance:
    ...
    def test_backfill_filters_by_team_ids(self, db_session):
        ...
        repo.backfill_appearance(
            mock_cv_func, season="2024", league="NCAA", team_ids=["100"]
        )
