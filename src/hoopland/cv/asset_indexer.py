...
class AssetIndexer:
    ...
    def analyze_file(self, file_path: str) -> List[AssetFeature]:
        ...
        if estimated_cols <= 1:
            print("  > Auto-detection failed. Falling back to Heuristic (11 items/row).")
            estimated_cols = 11
        ...
        for row_idx in range(rows):
            for col_idx in range(cols):
                ...
                if len(pixels) > 0:
                    ...
                    vol = (len(pixels) / (cell.transpose(2,0,1).shape[1] * cell.transpose(2,0,1).shape[2])) 
...
if __name__ == "__main__":
    ...
    indexer.run(r"c:\Users\73spi\mystuff\hoopland-v2\data\images", "hoopland_assets.json")
